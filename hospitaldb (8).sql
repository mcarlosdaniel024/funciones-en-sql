...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-03-2025 a las 09:10:51
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `ObtenerConsultorio` (`p_id_cita` INT) RETURNS INT(11) DETERMINISTIC BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `ObtenerNombreEnfermero` (`p_id_enfermero` INT) RETURNS VARCHAR(200) CHARSET utf8mb4 COLLATE utf8mb4_general_ci DETERMINISTIC BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `ObtenerNumeroCitas` (`p_id_paciente` INT, `p_fecha` DATE) RETURNS INT(11) DETERMINISTIC BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `TotalCitasPaciente` (`id_paciente` INT) RETURNS INT(11) DETERMINISTIC BEGIN
    DECLARE total INT;
    SELECT COUNT(*) INTO total FROM citas WHERE id_paciente = id_paciente AND estado = 'activa';
    RETURN total;
END$$
...
--
DELIMITER $$
CREATE TRIGGER `trg_auditoria_cancelacion` AFTER DELETE ON `citas` FOR EACH ROW BEGIN
    INSERT INTO auditoria_citas_canceladas (id_cita, paciente, motivo_cancelacion, fecha_cancelacion)
    VALUES (
        OLD.id_cita,
        (SELECT nombre_paciente FROM pacientes WHERE id_paciente = OLD.id_paciente),
        'Cancelación por procedimiento',
        NOW()
    );
END
$$
